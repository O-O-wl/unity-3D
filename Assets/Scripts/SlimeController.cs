...
public class SlimeController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (setOn)
        {
            ...
            if(skillCoolTime<skillDeltaTime){
                skillDeltaTime = 0;

                GameObject attack = Instantiate(SlimeAttack) as GameObject;
                attack.transform.position = new Vector3(transform.position.x, 1f, transform.position.z);
            }
            destination = new Vector3(GameObject.Find("Player").gameObject.transform.position.x, 0, GameObject.Find("Player").gameObject.transform.position.z);
        }
